...
class RayPPOTrainer(object):
                            ...
                            with _timer('verify', timing_raw):
                                ...
                                if 'mix' in self.config.reward_model.reward_manager:
                                    ...
                                    batch.meta_info['reward_config'] = {
                                        'shaping_function_name': self.config.reward_model.get('shaping_function_name', 'identity'),
                                    ...
                                    batch.pop(['actor_hidden_states','actor_answer_mask','golden_answer_mask','golden_hidden_states',
                                    'gt_answer_span','max_gt_ans_len','max_pred_ans_len','pred_answer_span'])
                                else:
    ...
    def replace_answer_with_gt_batch_old(self, gen_ids_batch, gen_response_batch, ground_truth_batch,
        ...
        for i in range(batch_size):
            if not valid_flags[i] or torch.sum(batch_input_ids[i] != self.tokenizer.pad_token_id).item() > max_length:
                valid_flags[i] = False
    ...
    def replace_answer_with_gt_batch(self, gen_ids_batch, gen_response_batch, ground_truth_batch,
        ...
        gen_texts_rmpad = [replace_left_and_right_str(text, pad_token_str) for text in gen_texts]
        # response
        gen_response_texts_rmpad = [replace_right_str(text, pad_token_str) for text in gen_response_texts]
        ...
        for i in range(batch_size):
            ...
            if valid_flag:
                ...
                # 验证答案部分：必须非空且包含结束标签
                if not answer_section.strip() or (end_answer not in answer_section):
                    valid_flag = False
                else:
                    # 提取预测答案文本（去除结束标签后的内容）
                    pred_answer_texts[i] = answer_section.split(end_answer)[0].strip()
                    
                    # 提取前导空白字符（答案开始前的空格、换行等）
                    leading_whitespace = ''
                    for char in answer_section:
                        if char in [' ', '\n', '\t', '\r']:
                            leading_whitespace += char
            ...
            if valid_flag:
                ...
                end_text_ids = self.tokenizer.convert_tokens_to_ids(self.tokenizer.tokenize(end_text))
                rm_pad=replace_left_and_right_str(self.tokenizer.decode(gen_ids_batch[i][:-len(end_text_ids) - 5],
                                                  skip_special_tokens=False), self.tokenizer.pad_token) 
                                        
                rm_end=replace_left_and_right_str(rm_pad, self.tokenizer.eos_token) 
                # new_text = (replace_left_and_right_str(self.tokenizer.decode(gen_ids_batch[i][:-len(end_text_ids) - 5],
                #                                   skip_special_tokens=False), self.tokenizer.pad_token) +
                #             end_text)
                new_text = rm_end + end_text
        ...
        # 批量 tokenize 新文本
        new_text_max_length=max_length+1024 # 有可能golden answer过长导致超出max_length prompt_length+response_length+1024
        # batch_input_data = self.tokenizer(new_texts, return_tensors='pt',
        #                                   add_special_tokens=False, truncation=True,
        #                                   max_length=max_length + self.config.data.max_response_length)
        batch_input_data = self.tokenizer(new_texts, return_tensors='pt',
                                          add_special_tokens=False, truncation=True,
                                          max_length=new_text_max_length)
        batch_input_ids = batch_input_data['input_ids']
        batch_attention_mask = batch_input_data['attention_mask']
        ...
        # >>> ADD: locate pred in response, then shift by max_prompt_length
        pos_pred_resp = self.batch_locate_substring_tokens(gen_response_texts_rmpad, pred_answer_texts)
        pred_answer_span = []
        max_pred_ans_len = 0
        for i in range(batch_size):
            if valid_flags[i] and pos_pred_resp[i]:
                s, e = pos_pred_resp[i][0], pos_pred_resp[i][-1]
                if e-s+1<=max_answer_length:
                    pred_answer_span.append([prompts_batch_shape + s, prompts_batch_shape + e])
                else:
                    # pred_answer_span.append([prompts_batch_shape + s, prompts_batch_shape + s+max_answer_length-1])
                    pred_answer_span.append([prompts_batch_shape + e-max_answer_length+1, prompts_batch_shape + e])
                # max_pred_ans_len = max(max_pred_ans_len, e - s + 1)
            else:
                pred_answer_span.append([-1, -1])
        pred_answer_span = torch.tensor(pred_answer_span, dtype=torch.int)
        # <<< ADD
        # 处理 gt_tokens_one_more 配置
        if self.config.reward_model.get("gt_tokens_one_more", False):
            for pos_gt in pos_gt_batch:
        ...
        # >>> ADD: gt span in batch_input_ids coord
        gt_answer_span = []
        max_gt_ans_len = 0
        # <<< ADD
        for i in range(batch_size):
            if not valid_flags[i] or torch.sum(batch_input_ids[i] != self.tokenizer.pad_token_id).item() > new_text_max_length:
                valid_flags[i] = False
            # pos_startanswer_batch 最后一个<answer> token 在new_texts出现的位置idx
            pos_startanswer = pos_startanswer_batch[i] 
            pos_gt = pos_gt_batch[i] # ground truth 在new_texts中出现的位置idx

            if pos_startanswer and pos_gt and len(pos_startanswer) > 0 and len(pos_gt) > 0:
                start_pos = pos_startanswer[0] # <answer> token 开始位
                gt_start = pos_gt[0] # ground truth 在new_texts开始位
                gt_end = pos_gt[-1] # ground truth在new_texts的结束位 但没有padding

                prompts = batch_input_ids[i:i + 1, :start_pos] # 注意这里认为 <answer> token之前的部分都是prompt (不包括<answer> token)
                responses = batch_input_ids[i:i + 1, start_pos:] # 这里认为<answer> token 及其之后的部分都是response

                ground_truth_ids = batch_input_ids[i:i + 1, gt_start:gt_end + 1]
                start = gt_start - start_pos

                # Padding
                left_pad_tuple = (max_length - prompts.shape[-1], 0) # 注意这里的prompt其实是<answer> token 之前的所有内容，所以最长是max_length
                # left_pad_tuple = (self.config.data.max_prompt_length - prompts.shape[-1], 0)
                # right_pad_tuple = (0, self.config.data.max_response_length+1024 - responses.shape[-1])
                # right_pad_tuple = (0, self.config.data.max_response_length - responses.shape[-1])
                right_pad_tuple = (0, 1024 - responses.shape[-1])
                ...
                ground_truth_mask = torch.zeros_like(responses)
                if valid_flags[i]:
                    # ground_truth_mask是相对于response来说的
                    ground_truth_mask[:, start:start + ground_truth_ids.shape[-1]] = 1
                # >>> ADD
                if gt_end-gt_start+1<=max_answer_length:
                    gt_answer_span.append([gt_start+left_pad_tuple[0], gt_end+left_pad_tuple[0]])
                else:
                    gt_answer_span.append([gt_end-max_answer_length+1+left_pad_tuple[0], gt_end+left_pad_tuple[0]])
                    # gt_answer_span.append([gt_start+left_pad_tuple[0], gt_start+max_answer_length-1+left_pad_tuple[0]])
                # max_gt_ans_len = max(max_gt_ans_len, gt_end - gt_start + 1)
                # <<< ADD
            else:
                # 创建默认值
                prompts = torch.zeros(1, max_length, dtype=torch.long)
                responses = torch.zeros(1, 1024, dtype=torch.long)
                input_ids = torch.cat([prompts, responses], dim=-1)
                attention_mask = torch.zeros_like(input_ids)
                position_ids = torch.zeros_like(input_ids)
                ground_truth_mask = torch.zeros_like(responses)
                gt_answer_span.append([-1, -1])
        ...
        # >>> ADD: attach 4 fields
        batch_results["pred_answer_span"] = pred_answer_span  # [B,2] in gen_ids_batch coord
        batch_results["gt_answer_span"] = torch.tensor(gt_answer_span, dtype=torch.int)  # [B,2] in batch_input_ids coord
        # batch_results["max_pred_ans_len"] = torch.tensor([max_pred_ans_len] * batch_size, dtype=torch.int)
        batch_results["max_pred_ans_len"] = torch.tensor([1024] * batch_size, dtype=torch.int)
        # batch_results["max_gt_ans_len"] = torch.tensor([max_gt_ans_len] * batch_size, dtype=torch.int)
        batch_results["max_gt_ans_len"] = torch.tensor([1024] * batch_size, dtype=torch.int)
        # <<< ADD
        return batch_results
