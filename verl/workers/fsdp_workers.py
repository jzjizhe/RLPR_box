...
class ActorRolloutRefWorker(Worker):
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            is_get_actor_hidden=((self.config.actor.is_get_hidden and (not self.config.actor.get("is_ref_hidden",False)))
            or (self.config.actor.is_get_hidden and self.config.actor.get("merge_golden_method",False)=="cot_answer"))
            output, entropy , actor_hidden_states, actor_answer_mask = self.actor.compute_log_prob(data=data,get_hidden_state=is_get_actor_hidden,
            layer_list=self.config.actor.get("layer_list",None))
            if is_get_actor_hidden:
                output = DataProto.from_dict(
                    tensors={"old_log_probs": output, "entropy": entropy,
                    "gen_hidden_states":actor_hidden_states,
                    "gen_answer_mask":actor_answer_mask},
        ...
        return output
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_actor_golden_hidden_states(self, data: DataProto):
        assert self._is_actor
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
        data = data.to('cuda')
        ...
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data)
            actor_hidden_states, actor_answer_mask = self.actor.compute_golden_hidden_states(data=data,layer_list=self.config.actor.get("layer_list",None))
            output = DataProto.from_dict(
                tensors={
                "golden_hidden_states":actor_hidden_states,
                "golden_answer_mask":actor_answer_mask},
                meta_info={"temperature": self.config.rollout.temperature},
            )
            output = self.ulysses_sharding_manager.postprocess_data(output)
        output = output.to('cpu')
        ...
        # clear kv cache
        torch.cuda.empty_cache() # remove 1
        log_gpu_memory_usage('After compute_actor_golden_hidden_states', logger=logger)
        return output
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_actor_gen_hidden_states(self, data: DataProto):
        assert self._is_actor
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
        data = data.to('cuda')
        ...
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data)
            actor_hidden_states, actor_answer_mask = self.actor.compute_gen_hidden_states(data=data,layer_list=self.config.actor.get("layer_list",None))
            output = DataProto.from_dict(
                tensors={
                "gen_hidden_states":actor_hidden_states,
                "gen_answer_mask":actor_answer_mask},
    ...
    def compute_ref_golden_hidden_states(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data)
            ref_hidden_states, ref_answer_mask = self.ref_policy.compute_golden_hidden_states(data=data,layer_list=self.config.actor.get("layer_list",None))
            output = DataProto.from_dict(
                tensors={
                "golden_hidden_states":ref_hidden_states,
                "golden_answer_mask":ref_answer_mask},
                meta_info={"temperature": self.config.rollout.temperature},
            )
            output = self.ulysses_sharding_manager.postprocess_data(output)
        ...
        return output
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_ref_gen_hidden_states(self, data: DataProto):
        assert self._is_ref
        ...
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data)
            ref_hidden_states, ref_answer_mask = self.ref_policy.compute_gen_hidden_states(data=data,
                                                layer_list=self.config.actor.get("layer_list",None))
            output = DataProto.from_dict(
                tensors={
                "gen_hidden_states":ref_hidden_states,
                "gen_answer_mask":ref_answer_mask},
                meta_info={"temperature": self.config.rollout.temperature},
            )
            output = self.ulysses_sharding_manager.postprocess_data(output)
        ...
        return output
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_hidden_reward(self, data: DataProto):
        """
        在 Worker 进程中计算 hidden reward，使用 GPU 加速
        支持多卡并行处理，使用 ulysses_sharding_manager 进行数据分片和结果聚合
        """
        ...
        # 获取配置参数
        # 优先从 data.meta_info 获取，如果没有则从 self.config 获取
        if 'reward_config' in data.meta_info:
            reward_config = data.meta_info['reward_config']
        else:
            reward_config = self.config.get('reward_model', {})
        
        shaping_function_name = reward_config.get('reward_manager_shaping_function_name', 'identity')
        ...
        if shaping_function_name == 'identity':
            shaping_function = lambda x: x
        elif shaping_function_name == 'exp':
            shaping_function = partial(amplify_reward, alpha=alpha)
        elif shaping_function_name == 'exp_norm':
            shaping_function = partial(exp_norm, alpha=alpha)
        elif shaping_function_name == 'clip_01':
            shaping_function = partial(clip_01)
        elif shaping_function_name == 'random':
            shaping_function = lambda x: random.random()
        else:
            raise NotImplementedError(f"Unknown shaping function: {shaping_function_name}")

        # 将数据移动到 GPU
        data = data.to('cuda')

        # 使用 ulysses_sharding_manager 进行多卡并行处理
        with self.ulysses_sharding_manager:
            # 预处理数据：将数据分片到各个 GPU
            data = self.ulysses_sharding_manager.preprocess_data(data)
            ...
            # 获取 prompt 长度（假设所有样本的 prompt 长度相同）
            prompt_length = data[0].batch['prompts'].shape[-1] if batch_size > 0 else 0
            
            # 获取 hidden states 和 masks（数据已经在 GPU 上，无需再次移动）
            golden_hidden = data.batch['golden_hidden_states'][:,0,:,:]
            pred_hidden = data.batch['gen_hidden_states'][:,0,:,:]
            golden_mask = data.batch['golden_answer_mask']
            pred_mask = data.batch['gen_answer_mask']

            # 对当前 GPU 上的每个样本计算 reward
            for i in range(batch_size):
                ...
                # 解码 response
                predict_str = self.tokenizer.decode(valid_response_ids, skip_special_tokens=False)
                format_score = format_reward(predict_str=predict_str, format_mode=format_mode)

                # 计算 hidden score
                if self.config.actor.reward_hidden_type=="subspace_energy_overlap_per_sample":
                ...
                elif self.config.actor.reward_hidden_type=="subspace_energy_overlap_topk":
                    hidden_score = subspace_energy_overlap_topk(
                        golden_hidden=golden_hidden[i],
                        pred_hidden=pred_hidden[i],
                        golden_mask=golden_mask[i],
                        pred_mask=pred_mask[i],
                ...
                elif self.config.actor.reward_hidden_type=="subspace_energy_overlap_tokencenter_topk":
                    hidden_score = subspace_energy_overlap_tokencenter_topk(
                        golden_hidden=golden_hidden[i],
                        pred_hidden=pred_hidden[i],
                        golden_mask=golden_mask[i],
                        pred_mask=pred_mask[i],
                ...
                elif self.config.actor.reward_hidden_type=="subspace_sim":
                    hidden_score = subspace_sim(
                        golden_hidden=golden_hidden[i],
                        pred_hidden=pred_hidden[i],
                        golden_mask=golden_mask[i],
                        pred_mask=pred_mask[i],
                    )
                elif self.config.actor.reward_hidden_type=="subspace_sim_r_weighted_golden_nodiv":
                    hidden_score = subspace_sim_r_weighted_golden_nodiv(
                        golden_hidden=golden_hidden[i],
                        pred_hidden=pred_hidden[i],
                        golden_mask=golden_mask[i],
                        pred_mask=pred_mask[i],
                    )
                elif self.config.actor.reward_hidden_type=="subspace_energy_overlap_golden_anchored_topk":
                    hidden_score = subspace_energy_overlap_golden_anchored_topk(
                        golden_hidden=golden_hidden[i],
                        pred_hidden=pred_hidden[i],
                        golden_mask=golden_mask[i],
                        pred_mask=pred_mask[i],
                ...
                elif self.config.actor.reward_hidden_type=="subspace_energy_overlap_topk_symmetric_norm":
                    hidden_score = subspace_energy_overlap_topk_symmetric_norm(
                        golden_hidden=golden_hidden[i],
                        pred_hidden=pred_hidden[i],
                        golden_mask=golden_mask[i],
                        pred_mask=pred_mask[i],
                ...
                elif self.config.actor.reward_hidden_type=="subspace_energy_overlap_topk_white_v3":
                    hidden_score = subspace_energy_overlap_topk_white_v3(
                        golden_hidden=golden_hidden[i],
                        pred_hidden=pred_hidden[i],
                        golden_mask=golden_mask[i],
                        pred_mask=pred_mask[i],
                ...
                elif self.config.actor.reward_hidden_type=="subspace_energy_overlap_topk_white_v2":
                    hidden_score = subspace_energy_overlap_topk_white_v2(
                        golden_hidden=golden_hidden[i],
                        pred_hidden=pred_hidden[i],
                        golden_mask=golden_mask[i],
                        pred_mask=pred_mask[i],
                ...
                hidden_score = shaping_function(hidden_score)

                # 组合 score
                if format_coefficient == -1:
                    score = hidden_score if format_score == 1 else -1
                else:
                    score = (1 - format_coefficient) * hidden_score + format_coefficient * format_score
            ...
            # 构建输出 DataProto
            # 注意：hidden_score_tensor 在 ray_trainer.py 中被当作 scoreB_tensor 使用
            output = DataProto.from_dict(
                tensors={
                    'reward_tensor': reward_tensor,
                    'scoreB_tensor': hidden_score_tensor,  # 使用 scoreB_tensor 作为 key，以匹配 ray_trainer.py 的期望
                    'scoreA_tensor': scoreA_tensor,
