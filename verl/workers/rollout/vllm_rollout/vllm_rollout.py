...
class vLLMRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        if prompts.meta_info.get('validate', False):
            # kwargs.update({'n': prompts.meta_info.get('n', 1),'seed': prompts.meta_info.get('seed', 42), 'top_p': prompts.meta_info.get('top_p', 1.0),'temperature':  prompts.meta_info.get('val_temperature', 1)})
            if do_sample:
                kwargs.update({'n': prompts.meta_info.get('n', 1),'seed': prompts.meta_info.get('seed', 42), 'top_p': prompts.meta_info.get('top_p', 1.0),'temperature':  prompts.meta_info.get('val_temperature', 1)})
            else:
                kwargs.update({'seed': prompts.meta_info.get('seed', 42)})
