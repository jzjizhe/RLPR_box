    ...
    def __call__(self, data: DataProto, name=None):
        ...
        with multiprocessing.Pool(processes=32) as pool:
            ...
            for i in range(len(data)):
                ...
                result_list.append((result, i, data_source, sequences_str, ground_truth,extra_info))

                #### END
            pool.close()
            pool.join()

        for (result, i, data_source, sequences_str, ground_truth,extra) in result_list:
            ...
            if self.save_results_dir is not None:
                if "SuperGPQA" in data_source:
                    sequences_data.append({'data_source': data_source, 'sequences_str': sequences_str, 
                                        ...
                                        "from_judge": from_judge, "judge_response": judge_response,
                ...
                elif data_source=="MMLUProALL":
                    sequences_data.append({'data_source': data_source, 'sequences_str': sequences_str, 
                                        'extracted_answer': extracted_answer, 'ground_truth': str(ground_truth), 
                                        'score': score, 'format_score': format_score, "acc": acc, 
                                        "from_judge": from_judge, "judge_response": judge_response,
                                        "category":extra['category']}) #, "judge_prompt": judge_prompt})
                else:
                    sequences_data.append({'data_source': data_source, 'sequences_str': sequences_str, 
                                        'extracted_answer': extracted_answer, 'ground_truth': str(ground_truth), 
                                        'score': score, 'format_score': format_score, "acc": acc, 
