...
def get_token_hidden_states(hidden_states_ls, span, max_length):
    """
    提取 answer token 的 hidden states（跨多个层），支持 ragged span → padded output。

    Args:
        hidden_states_ls: list of tensors, 每个 tensor 形状:
            [batch, seq_len, dim]
            例如来自 model 的全部层 hidden states。
        span: LongTensor [batch, 2]
            每个样本的 (start, end)，若为 [-1,-1] 表示无答案区域。
    
    Returns:
        answer_hidden: Tensor [batch, num_layers, max_answer_len, dim]
        answer_mask: Tensor  [batch, max_answer_len]
    """
    num_layers = len(hidden_states_ls)
    batch, seq_len, dim = hidden_states_ls[0].shape

    # span unpack
    start = span[:, 0]   # [B]
    end   = span[:, 1]   # [B]
    max_len = max_length.item()

    # 建立输出张量
    answer_hidden = torch.zeros(batch, num_layers, max_len, dim, device=hidden_states_ls[0].device)
    answer_mask   = torch.zeros(batch, max_len, dtype=torch.bool, device=hidden_states_ls[0].device)

    for b in range(batch):
        s = start[b].item()
        e = end[b].item()

        if s < 0 or e < 0:   # span = [-1, -1]
            continue

        L = e - s + 1
        answer_mask[b, :L] = 1

        for layer_id, layer_h in enumerate(hidden_states_ls):
            # layer_h: [batch, seq_len, dim]
            answer_hidden[b, layer_id, :L, :] = layer_h[b, s:e+1, :]
...
class DataParallelPPOActor(BasePPOActor):
    ...
    def __init__(
        ...
        self.layer_list = self.config.get("layer_list", None)
    def _forward_micro_batch(self, micro_batch, temperature,output_hidden_states=False,layer_list=None):
        """
        Returns: 
        ...
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                if self.use_ulysses_sp:
                    ...
                    if output_hidden_states:
                        for i in range(len(hidden_states_ls)):
                            hidden_states_ls[i] = gather_outpus_and_unpad(
                                hidden_states_ls[i],
                                gather_dim=0,
                                unpad_dim=0,
                                padding_size=pad_size,
                ...
                if output_hidden_states:
                    hidden_states_ls,token_mask=get_token_hidden_states(hidden_states_ls,micro_batch['answer_span'],micro_batch['max_ans_len'][0])
    ...
    def _forward_micro_batch_get_hidden(self, micro_batch, temperature,output_hidden_states=False,layer_list=None):
        """
        Returns: 
            entropy: # (bs, response_len)
            log_probs: # (bs, response_len)
        ...
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
            ...
            return  hidden_states_ls,token_mask
    ...
    def compute_log_prob(self, data: DataProto,get_hidden_state=False,layer_list=None) -> torch.Tensor:
        ...
        if get_hidden_state:
            select_keys.append("gen_answer_span_pr")
            select_keys.append("max_gen_ans_len_pr")
        batch = data.select(batch_keys=select_keys).batch
        if get_hidden_state:
            batch['answer_span']=batch.pop('gen_answer_span_pr')
            batch['max_ans_len']=batch.pop('max_gen_ans_len_pr')
        if use_dynamic_bsz:
        ...
        for micro_batch in micro_batches:
            with torch.no_grad():
                ...
                entropy, log_probs,hidden_states_ls,answer_mask = self._forward_micro_batch(micro_batch, temperature=temperature,
                                    output_hidden_states=get_hidden_state,layer_list=layer_list)
        ...
        if use_dynamic_bsz:
            ...
            if get_hidden_state:
                ...
                answer_mask=answer_mask[revert_indices]
        # if get_hidden_state:
        return log_probs, entropy,hidden_states,answer_mask
        # else:
            # return log_probs, entropy

    def compute_golden_hidden_states(self, data: DataProto,layer_list=None) -> torch.Tensor:
        # set to eval
        self.actor_module.eval()
        ...
        select_keys = ['input_ids_pr', 'attention_mask_pr', 'position_ids_pr', "gt_answer_span_pr","max_gt_ans_len_pr"]
        batch = data.select(batch_keys=select_keys).batch
        batch['input_ids']=batch.pop('input_ids_pr')
        batch['attention_mask']=batch.pop('attention_mask_pr')
        batch['position_ids']=batch.pop('position_ids_pr')
        batch['answer_span']=batch.pop('gt_answer_span_pr')
        batch['max_ans_len']=batch.pop('max_gt_ans_len_pr')
        ...
        for micro_batch in micro_batches:
            with torch.no_grad():
                hidden_states_ls,answer_mask = self._forward_micro_batch_get_hidden(micro_batch, temperature=temperature,
                                    output_hidden_states=True,layer_list=layer_list)
            hidden_states_lst.append(hidden_states_ls)
            answer_mask_lst.append(answer_mask)
        hidden_states=torch.concat(hidden_states_lst,dim=0)
        answer_mask=torch.concat(answer_mask_lst,dim=0)
        ...
        if use_dynamic_bsz:
            ...
            hidden_states=hidden_states[revert_indices]
            answer_mask=answer_mask[revert_indices]
        return hidden_states,answer_mask

    def compute_gen_hidden_states(self,layer_list ,data: DataProto) -> torch.Tensor:
        # set to eval
        self.actor_module.eval()
        ...
        try:
            select_keys = ['gen_input_ids_pr', 'gen_attention_mask_pr', 'gen_position_ids_pr', "gen_answer_span_pr","max_gen_ans_len_pr"]
            batch = data.select(batch_keys=select_keys).batch
            batch['input_ids']=batch.pop('gen_input_ids_pr')
            batch['attention_mask']=batch.pop('gen_attention_mask_pr')
            batch['position_ids']=batch.pop('gen_position_ids_pr')
            batch['answer_span']=batch.pop('gen_answer_span_pr')
            batch['max_ans_len']=batch.pop('max_gen_ans_len_pr')
        except:
            select_keys = ['input_ids_pr', 'attention_mask_pr', 'position_ids_pr', "gen_answer_span_pr","max_gen_ans_len_pr"]
            batch = data.select(batch_keys=select_keys).batch
            batch['input_ids']=batch.pop('input_ids_pr')
            batch['attention_mask']=batch.pop('attention_mask_pr')
            batch['position_ids']=batch.pop('position_ids_pr')
        ...
        for batch_idx, data in enumerate(dataloader):
            ...
            for data in micro_batches:
                ...
                if mode == 'normal':
                    ...
                    entropy, log_prob,_,_ = self._forward_micro_batch(micro_batch=data, temperature=temperature)
